...
# if not os.path.exists(os.path.join(results_dir, input_file_name.replace("_case", suffix)+".pickle")):
run_pypsa(network, input_file_name+".xlsx", case_dict, component_list, outfile_suffix=suffix)
    # network.export_to_csv_folder(os.path.join(results_dir, input_file_name.replace("_case", suffix)+".nc"))
    # logging.info("Saved network to {}".format(os.path.join(results_dir, input_file_name.replace("_case", suffix)+".nc")))
# else:
#     network = pypsa.Network()
#     network.import_from_csv_folder(os.path.join(results_dir, input_file_name.replace("_case", suffix)+".nc"))   
#     logging.info("Loaded network from {}".format(os.path.join(results_dir, input_file_name.replace("_case", suffix)+".nc")))
...
while keep_removing == True: 
    ...
    result_file_name = input_file_name.replace("_case", suffix)
    # Load results
